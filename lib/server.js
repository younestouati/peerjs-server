  ...
  socket.on('message', function(data) {
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
        ...
        if (message.name !== 'heartbeat') { //Custom addition to support heart beat pings.
...
app._generateClientId = function(key) {
  ...
  while (!!this._clients[key][clientId]) { //TODO: LOOK INTO THIS. COULD BE A PROBLEM IF THERE ARE A LOT OF SESSIONS!
